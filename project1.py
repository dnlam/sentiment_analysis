...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        loss = loss + np.maximum(0, 1-(labels[i]*(np.dot(feature_matrix[i],theta) +theta_0)))
        print(loss)
...
def perceptron_single_step_update(
    ...
    if label*(np.dot(feature_vector,current_theta)+current_theta_0) <=0:
        return current_theta+np.dot(label,feature_vector), current_theta_0+label
    else:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            nT = nT + 1
    return sum_theta/nT, sum_theta_0/nT
#pragma: coderesponse end
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        if np.dot(feature_matrix[i], theta) +  theta_0 > 0:
            pred_outout.append(1)
        else:
            pred_outout.append(-1)
    return np.array(pred_outout)
    #raise NotImplementedError
#pragma: coderesponse end
